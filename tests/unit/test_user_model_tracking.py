...
class TestFacts:
    ...
    def test_fact_reinforced_by_new_source(self):
        um.note_fact("bio", "lives in Denver", confidence=0.6, source="dream:2026-06-30")
        um.note_fact("bio", "lives in Denver", confidence=0.6, source="dream:2026-07-01")
        facts = [f for f in um._recent_facts(50) if "Denver" in f["text"]]
        assert len(facts) == 1
