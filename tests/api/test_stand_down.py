...
The button in Settings > Models was a placebo. It POSTed to /api/machine/level,
which returned, in a raw browser `alert()`:

    "Noted, but nothing enforces machine levels yet -- working/away/yield is a
     decision the maintainer has not made (headroom.md D1). This click does not
     release or stand anything down."

Honest, and useless. The route said so itself: "STUBBED, deliberately ... D1 is
not decided". This implements the D1 answer relayed 2026-09-24:

    "I need my machine" releases the machine to him. Local models are unloaded
    from the GPU (Laya is CPU, so it stays), every background and scheduled job
    is paused, and the header shows "Friday is stood down -- Resume". Interactive
    chat still works on a cloud seat with its usual visible model label, or says
    it is waiting if cloud is off. The state lasts until Resume, or auto-resumes
    after N hours of his choosing. Background jobs never wake the GPU while stood
    down.
...
def test_auto_resume_expires_on_its_own(sd):
    sd.stand_down(requested_by="stephen", hours=2)
    st = sd.state()
...
def test_no_auto_resume_means_it_waits_for_resume(sd):
    sd.stand_down(requested_by="stephen", hours=None)
...
def test_the_same_job_runs_once_resumed(sd, monkeypatch):
    ...
    sd.stand_down(requested_by="stephen")
    sd.resume(requested_by="stephen")
    scheduler._run_task(rec)
