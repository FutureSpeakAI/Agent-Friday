...
DEMO-MODE NOTE: _generate_text() has a demo-mode gate
(services/model_router.py, just above the router consult) that returns a
canned placeholder EARLY -- before the router is ever consulted --
whenever demo_mode.is_demo() is true, which it is by default whenever no
provider has a usable key/model. Without patching that gate, a machine
with no ambient provider key (a fresh clone, CI, a sandboxed run) would
return the placeholder before reaching the refuse/vault_access logic, and
every assertion below would pass vacuously (zero provider calls, because
NOTHING was called). Every test here therefore forces demo_mode.is_demo()
to False, so the routed code path is exercised on any machine.
"""
...
def _patch_router(monkeypatch, result):
    ...
    # in its ambient environment -- see the DEMO-MODE note above.
