"""Gauntlet finding F13 (findings.jsonl F13).
...
The risk is that a build regenerating index.html from this mirror would
silently lose them. src/agent_friday/ui/build_ui.py carries a REGRESSION
GUARD against that: the assembler diffs top-level component names in the
...
unless a human explicitly passes --force. That guard covers every component
missing from the mirror, not just the ones pinned here.

Of the three possible resolutions — port the components, retire the mirror,
or lock down the build path so a regeneration can never silently destroy
shipped code — the third is the one in force. This probe proves it: it runs
the real build script against the real repo (safe, because the guard's whole
job is to refuse to write) and asserts it actually refuses, names the
at-risk components, and leaves index.html byte-for-byte untouched. That is the concrete, checked
...
def test_build_ui_refuses_to_silently_drop_shipped_components():
    ...
    assert after == before, (
        ...
        "regression guard is no longer protecting the file the "
