...
Of the 14 currently missing, four are high-severity because they are either
constantly-visible core chrome or have a documented history of costing real
money or blocking onboarding when they've gone missing before:

  - ConsentFlow       -- the entire first-run onboarding wizard, including
                         vault passphrase collection
  - SettingsTabCosts  -- the cost/budget panel (previously deleted for two
                         months; real spend ran to $1,189.76/mo against a
                         $50 budget before it was restored)
  - ConversationBar   -- persistent chat-window chrome (conversation title,
                         per-chat model pin)
...
RED: porting four React components into a hand-maintained mirror
unattended, overnight, is not a call to make unilaterally -- it needs
Stephen to decide whether to port them, delete the mirror file entirely (if
nothing depends on it), or accept the drift and make sure no build path can
ever regenerate index.html from it (KNOWN_ISSUES.md's §2 history records
that exact incident happening once already). This probe should stay red
until one of those happens; a change that just deletes app.html or
suppresses this test without addressing the underlying question is not a
fix.
"""
