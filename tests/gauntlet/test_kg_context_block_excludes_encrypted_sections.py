...
CASING NOTE: the candidate's `section` is "Private" (mixed case) while
the mocked encrypted-sections entry is "private". wiki_graph.py stores
`section` as the raw path segment (`rel.split("/")[0]`, never lowercased)
-- exactly what a real wiki folder name looks like on disk -- whereas
`_wiki_encrypted_sections()` always returns a lowercased set. Using
lowercase on both sides could never catch a fix that compares the raw,
case-preserved `section` directly against that set and so fails to
exclude a directory named "Private".
"""
