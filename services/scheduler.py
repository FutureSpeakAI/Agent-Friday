...
import core
from core import FRIDAY_DIR, _load_settings, process_register, process_update
...
def _notify_run(rec, status, summary):
    ...
    try:
        from services.voice_engine import _notif_engine as _ne
    except Exception:
        _ne = None
    if not _ne:
        return
...
def _run_task(rec):
    ...
    from services.agent import _spawn_task, _task_snapshot
...
def _register_default_builtin_tasks():
    ...
    try:
        from services.creations import generate_daily_creation
    ...
    try:
        from services.news_engine import _run_front_page_job, FRONT_PAGE_SLOTS
    ...
    try:
        from services.news_engine import (_run_weekly_digest_job,
    ...
    try:
        from services.notifications import _run_self_improvement_job
    ...
    try:
        from services.model_router import _run_session_summary_job
    ...
    try:
        from services.repo_sync import run_repo_sync
...
def _afternoon_briefing_job():
    ...
    from services.news_engine import _gather_live_briefing_context, _notify_briefing
