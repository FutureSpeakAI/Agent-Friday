...
| **Private / family data** | Personal exports, anything naming a **minor** | Never in-repo. Store under `~/.friday/`; it is gitignored |
...
- [ ] No tracked `*.bat` / `*.vbs` / `.env` / private data files: `git ls-files | grep -iE "\.(bat|vbs|env)$|household|bridge-token"` is empty.
