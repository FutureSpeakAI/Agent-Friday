...
def set_researcher(fn) -> None:
    """Wire a researcher into the auto-research loop.

    Signature: (skill_name, context) -> {"hypotheses": [...], "edits": [...]}.
    Defined here so an UPPER layer (services) can inject the LLM call — this
    leaf module must never import upward itself. Pass None to revert to the
    built-in heuristics.
    """
    get_engine().research.researcher = fn
