...
from services.prompt_manager import SEGMENT_KEYS, PromptManager
...
def api_health_full():
    ...
    out = {"status": "ok"}
    try:
        out["server"] = {"uptime_seconds": int(_t.time() - getattr(core, "SERVER_START_TS", _t.time()))}
    ...
    try:
        from services.agent import _load_mcp_servers
