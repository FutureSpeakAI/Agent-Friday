...
- **Originally `server.py` never imported it** — vault data (`finance`, `health`, `ofw`, `vault/legal`…)
...
- **This-repo-only (absent from website):** the holographic UI, Liquid UI app generation, `job_scanner`,
  `application_engine`, OFW/co-parenting monitor, daily creation, vibe-code swarm, Seeds & Gardens.
...
| **Business positioning** | Site company pages frame FutureSpeak.AI as an **enterprise marketing-AI agency** ("save 90% on marketing") with a **paid certification program** ($500–$10,000; Core/Connected/Sovereign). This repo frames the product as **"sovereign AI for everyone… not just enterprises," MIT, free.** Opposite go-to-market. |
| **Founder bio** | Consistent core (Webster, 20+ yrs, Raw Story EIC, Aquent, trained data for Google/Meta/Amazon). Variances: site adds "Founder & CEO" and "founded Austin.com"; this repo's `VOICE_DEMO.md` adds claims the site omits ("Raw Story 50k→5M," "Sanofi €120M," "built SAGE," "49 repos"). Site says "**16+** repositories" vs `VOICE_DEMO`'s "**forty-nine**." |
