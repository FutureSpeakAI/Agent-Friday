...
| Seam | Status | Notes |
|---|---|---|
| Settings write → settings read | SWEEP 2 FOUND NEW (F14, F15, both fixed) — needs a 3rd clean sweep | Round 1: F3 (context_retention_days, queued+probe-pinned), F6 (camera_auto_describe, dead, low-priority), 7 HOLDS confirmed. Round 3: fresh-context second sweep dispatched (Voice/Creative/Scheduler/Notification/Connector settings panels specifically, outside what round 1 covered). |
