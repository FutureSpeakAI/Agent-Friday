...
| Seam | Status | Notes |
|---|---|---|
| Settings write → settings read | SWEEP 4 FOUND NEW (F24: persona/distribution presets mostly placebo, queued — onboarding-facing) — needs a 5th clean sweep | Round 1: F3 (context_retention_days, queued+probe-pinned), F6 (camera_auto_describe, dead, low-priority), 7 HOLDS confirmed. Round 3: fresh-context second sweep dispatched (Voice/Creative/Scheduler/Notification/Connector settings panels specifically, outside what round 1 covered). |
...
| Router request → seat actually served | SWEEP 5 FOUND NEW (F30: fallback ladder reuses gated-for-wrong-provider prompt, queued, systemic; Q12: last_local_refusal race, latent/inert, observed) — needs a 6th clean sweep | Round 1: F2 (provider_health gaps, FIXED), H2/H3/H6 confirmed HOLDS. Round 3: fresh-context second sweep dispatched (_route_basic's other branches, _apply_cloud_provider substitution risk, other seat-guess functions besides effective_seat, stale conversation_seat overrides). |
...
| Voice pipeline end-to-end | SWEEP 4 FOUND NEW (F27: TTS ignored local_preferred, FIXED) — needs a 5th clean sweep | Round 1 (via the F4 investigation) confirmed the screenshot/vision-attachment sub-path respects local-only routing (H9, HOLDS, fixed 2026-08-23). Round 2's prompt-assembly finder also confirmed routes/voice.py's Gemini Live text/tool-result gating HOLDS. Broader voice pipeline (STT/TTS audio path itself, live voice barge-in, Tier-1/Tier-2 fallback, and Q6's unmetered Gemini TTS/Live billing gap) not yet fully swept. |
| Scheduler & workflows | SWEEP 2 FOUND NEW (F25: builtin tasks ship with zero retry tolerance, queued; Q15: away_drain has no UI on-switch, incomplete feature not a regression) — needs a 3rd clean sweep | Round 1 found + fixed the orphaned nightly KG reindex job (F1). Scheduler's other ~15 builtin tasks spot-checked incidentally but not exhaustively; routes/workflows.py and workflow_plan.py (the "workflows" half of this seam) not yet swept. |
...
| Knowledge graph ingest → retrieval | SWEEP 3 FOUND NEW (F31: Tier B extraction had no cap on cloud-eligible LLM calls per pass — live incident, 7+ hours/~$10-hour unattended spend, FIXED) — needs a 4th clean sweep | Round 1: F1 (nightly reindex orphaned, FIXED), H4 (indexer method-name fix confirmed correct and now reachable via the fix). Round 2's prompt-assembly finder additionally confirmed KG retrieval content is double-protected (sensitivity-based model pinning + full egress classification) before reaching cloud. Round 3 (live-incident investigation, see progress.md top section and F31): traced a real overnight cost leak to reindex_tier_b's uncapped extraction loop; added MAX_CLOUD_EXTRACT_CALLS, proven red-green-red. Q9/Q10 (forget-me description leak; no correction-invalidation + unbounded growth) still queued from round 2. Needs 1 more clean fresh-context sweep. |
| MCP and connector registration | SWEEP 3 FOUND NEW (F28: unbounded stdout/stderr memory growth, FIXED) — needs a 4th clean sweep | Round 2: F8 (restart() bypassed the security-disable gate, FIXED), F9 (status mislabeling for a security-blocked connector, queued — the observability half of the same cluster), H10 (registration→tool-callable chain, OAuth expiry disclosure, local-seat tool dropping disclosure, no orphaned registration function — all confirmed HOLDS). Needs 1 more clean fresh-context sweep. |
