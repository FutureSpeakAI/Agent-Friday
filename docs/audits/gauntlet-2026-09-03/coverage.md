...
| Seam | Status | Notes |
|---|---|---|
| Settings write → settings read | SWEEP 1 CLEAN, SWEEP 2 IN PROGRESS | Round 1: F3 (context_retention_days, queued+probe-pinned), F6 (camera_auto_describe, dead, low-priority), 7 HOLDS confirmed. Round 3: fresh-context second sweep dispatched (Voice/Creative/Scheduler/Notification/Connector settings panels specifically, outside what round 1 covered). |
...
| Voice pipeline end-to-end | IN PROGRESS, round 3 dispatched (checking whether Gemini TTS respects local-only) | Round 1 (via the F4 investigation) confirmed the screenshot/vision-attachment sub-path respects local-only routing (H9, HOLDS, fixed 2026-08-23). Round 2's prompt-assembly finder also confirmed routes/voice.py's Gemini Live text/tool-result gating HOLDS. Broader voice pipeline (STT/TTS audio path itself, live voice barge-in, Tier-1/Tier-2 fallback, and Q6's unmetered Gemini TTS/Live billing gap) not yet fully swept. |
| Scheduler & workflows | IN PROGRESS, round 3 dispatched (workflows.py/workflow_plan.py specifically) | Round 1 found + fixed the orphaned nightly KG reindex job (F1). Scheduler's other ~15 builtin tasks spot-checked incidentally but not exhaustively; routes/workflows.py and workflow_plan.py (the "workflows" half of this seam) not yet swept. |
