...
| `auto` **("Automatic (local only)")** | Tier2 (if ready) → Tier1 → text-only. **Never cloud.** | text-only + classified error |
| `gemini` (Tier 3) | Cloud (model attempt plan) → (local only if not local-only) → text-only | text-only + classified error |

| `cloud:<provider>` (Tier 3, ElevenLabs / Inworld) | provider -> **(announce)** Tier 2 if ready -> **(announce)** Tier 1 -> text-only | text-only + classified error |

**Amendment 2026-09-09 - the `auto` row.** The parenthetical "(cloud only if key present AND not local-only)" has been **deleted** from the `auto` row. It was in direct conflict with §7.3's own no-auto-promotion clause and with `cloud-voice-providers.md` §6.3, which forbids promoting to cloud on a local failure without consent. `auto` now means the best *local* tier that can serve, terminating at text-only, and is labelled **"Automatic (local only)"** in the interface so the word matches the behaviour. This resolves the voice-mode audit's open item 1 and Q5 of `local-voice-repair-and-native-audio.md` §4.5 as **R4.1**. Enforced in `routes/voice.py::_resolve_voice_engine` and pinned by `tests/gauntlet/test_resolve_engine_auto_never_cloud.py`, which fails if the cloud branch is ever restored.

**No-auto-promotion clause (§7.3, cloud direction).** A **local** failure never promotes to a cloud provider. It surfaces with its reason and *offers* the cloud path as an explicit action the user takes; declining leaves Friday in text-only. This is deliberately worse ergonomics than auto-promotion, because the local path failing is exactly the moment the user most needs to know their audio is about to leave the machine. The reverse direction (cloud fails -> local) is permitted and announced.
