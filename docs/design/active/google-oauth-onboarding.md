...
* the second user connects in one click. So do the next 99 people.
...
* But it is still *"create a Google Cloud project"*. Best case it takes a
  motivated person ten minutes. the second user would not have finished it, and the second user
  is the test.
...
the second user — and failing it is what produced this document. B is right in the
