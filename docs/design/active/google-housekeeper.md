...
**Re-check 2026-09-06 (doc-reconciliation pass):** a fourth verification, against the current tree and the live `~/.friday/google_accounts/accounts.json`, and **every status claim in this document still holds** — it neither overstates nor understates. HK-1/HK-4 are landed exactly as described (`agent.py:1298-1362`, dispatch `:3736-3739`, `delete_task` in `_ALWAYS_CONFIRM`); `GOOGLE_MULTI_SCOPES` (`google_accounts.py:74-78`) does carry the read-write `tasks` scope in code; **but both accounts on disk still show `status: needs_reauth` with only `tasks.readonly` + `gmail.readonly` granted, so nothing writes yet — the re-consent has still not happened.** The calendar primary-account-only bug (HK-2) is still present and unfixed (`calendar_write._service()` still resolves through `primary_credentials()`, no `account_id` on any calendar tool). No `delete_calendar_event` exists. No Gmail write scope (HK-6/HK-9) has been added. No commit since 2026-08-29 touches any of this. Nothing in this document needed correcting; this note exists so the next reader knows it was checked rather than assumed.
