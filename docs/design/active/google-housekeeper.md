...
**Prompted by:** Google Tasks can be read but never completed, so the to-do list only grows. The maintainer then asked for the full role: add/remove/edit events and to-dos; compose, send and delete email — "my Google accounts' housekeeper."
...
**Blast radius if wrong:** an event created/edited on the wrong calendar — annoying, reversible, and *less* likely than today's behaviour, which gets the account wrong by design whenever the maintainer means Work.
...
**Flags:** re-consent ×2 to activate — and per §2's sequencing rule, **land this scope constant (and HK-9's, per Q-H2) before the maintainer performs the now-mandatory Tasks reconnect**, even if the draft tools themselves ship later; the constant is one line and beats a second consent ceremony. **This is the recommended stopping point for week one** — Friday composes, the maintainer sends from Gmail, trust accrues with zero send risk.
...
3. Render `deny` distinctly in the process orb instead of collapsing to error (`routes/voice.py:541-542`; no UI currently renders `status:"deny"`). A silent ring denial cost the maintainer a day once; refusals become visible here.
...
"Housekeeper" as the maintainer means it — tidying unprompted — is a scheduling-layer claim on trust, not a set of verbs, and it gets its own spec later. What must be true first:
...
3. First housekeeper mode is **propose-only**: a scheduled job produces a tidy *report* (tasks that look done, mail that looks archivable, calendar orphans) as an approval card batch; the maintainer approves the batch; execution is item-by-item with receipts. The wiki propose/apply flow is the in-repo precedent. Autonomy beyond that is earned per-verb, per the maintainer's explicit say-so, starting with the reversible ones.
4. Send never becomes autonomous by default; Q-H4 is where the maintainer defines any narrow exceptions.
