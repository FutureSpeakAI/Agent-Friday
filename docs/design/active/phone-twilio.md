...
`services/agent.py`, Settings → Phone in `index.html` and `ui_parts/app.html`.
...
* **A text from the verified cell** runs one agent turn with
  ...
  in the taint ledger when `services/taint.py` exists (it is on
  `feat/injection-defense`, not yet merged). The reply goes to the verified
  cell, never to the sender's number, and through the egress gate.
...
desktop voice session, which another workstream is repairing, so the two
compete for it.
...
1. **Settings → Phone:** paste the account SID, the new API key SID and
...
5. **Settings → Phone → Public address:** `https://phone.<your-domain>`, then
