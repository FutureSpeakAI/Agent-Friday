...
class ConversationMemory:
    ...
    def recent(self, n=20, roles=None):
        """Return the most-recent stored turns, newest-first.

        Unlike search(), this is chronological (by indexed timestamp) rather
        than semantic — used by the self-improvement introspection tools, which
        analyse *what Friday recently said* rather than answer a query. Reads
        the whole collection's metadata then slices, so it is O(n) in the index
        size; fine for the periodic (weekly) cadence it is called at.

        roles  optional iterable to restrict to (e.g. ['friday']).
        """
        if not self._ensure():
            return []
        try:
            ...
            print(f"  [MEMORY] recent failed (non-fatal): {e}")
            return []
