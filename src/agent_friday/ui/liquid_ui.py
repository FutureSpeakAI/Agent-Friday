...
STATUS: EXPERIMENTAL / NOT WIRED. No production code imports this module —
the engine is complete and unit-tested (tests/unit/test_liquid_ui.py) but no
route or service invokes it, ui_parts/liquid_ui_panel.html is not part of the
UI build, and it is deliberately excluded from the PyInstaller bundle.
Revisit after the frontend componentization (Vite migration): wiring a
generative-UI engine into a Babel-monolith frontend is the wrong order of
operations.
...
class BuildArtifact:
    ...
    hot_reload_token: Optional[str] = None  # pragma: allowlist secret
...
class LiquidUIBuilder:
    ...
    def build(self, spec: FeatureSpec, *, dry_run: bool = False) -> BuildArtifact:
        ...
        if not dry_run:
            ...
            artifact.hot_reload_token = hashlib.sha1(  # pragma: allowlist secret
