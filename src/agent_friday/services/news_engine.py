...
NEWS_CATEGORIES = {
    ...
    "Local": {
        "color": "local",
        # The Local beat defaults to one metro's feeds (Austin, TX).
        # Every feed below is a live RSS/Atom source verified to return
        # same-day Austin stories:
        #   - KUT (Austin NPR, UT-Austin's Moody College) — index.rss
        #   - KXAN (NBC Austin affiliate) — /feed/
        #   - KVUE (ABC Austin affiliate) — local-desk syndication feed
        #   - The Texas Tribune (statewide, HQ'd in Austin) — main feed
        # Austin Monitor is deliberately absent: it stopped publishing in
        # 2025 and is not a live source.
        # To use a different city, replace the four feeds below and the
        # query string; nothing else in the pipeline assumes Austin.
...
_BRUTALIST_CATEGORY_RULES = [
    ("Local", re.compile(r"\b(austin|texas|travis county|atx)\b", re.I)),
