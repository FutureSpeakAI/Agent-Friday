...
class ClaudeCodeAdapter(BaseAdapter):
    ...
    def _run(self, aid: str, task: "WorkerTask"):
        try:
            ...
            # SECURITY: the system prompt must be built with an explicit
            # `vault_control` — _build_context_prompt only tier-gates when
            # vault_control is given (see model_router._get_friday_system_prompt's
            # docstring: "Defaults keep the legacy ungated behavior"), so a
            # bare `provider="auto"` call would put ungated vault content
            # into the prompt. This is a background orchestration worker
            # like agent._task_worker, so it gets the same treatment:
            # predict the destination on the real task prompt
            # (has_tools=True, this runs through _generate_agent's agentic
            # loop) and gate for it.
            #
            # That prediction picks ONE provider, but _generate_agent's own
            # fallback ladder can land on a DIFFERENT provider than predicted
            # when the first leg fails operationally; a prompt gated once for
            # the predicted provider would then be reused for the wrong
            # destination. `_sys_for` rebuilds the prompt for an EXPLICIT
            # provider and is passed as `system_builder` so every ladder leg
            # — first attempt and every fallback — is gated for the provider
            # it actually calls.
