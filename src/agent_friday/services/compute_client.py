...
Scope, stated precisely: services/orchestrator.py never imports or calls
anything in this module -- there is no automatic fallback where a
local-adapter failure triggers federation delegation.
find_providers/request_job/rate_provider/get_sent_jobs are each wired to a
route in routes/compute.py, so delegation is reachable as a manual/external
API call, not something the Orchestrator invokes on its own when it hits
local incapacity. await_result() has no caller of any kind -- the wired
routes poll /api/federation/compute/status/<job_id> once per call instead of
blocking here.
...
await_result(job_id, timeout)               → dict  (JobResult) -- see scope note above: no caller
